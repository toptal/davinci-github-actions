/******/ ;(() => {
  ...
  /******/ var __webpack_modules__ = {
    ...
    /***/ 7447: /***/ (
      ...
      const external_node_buffer_namespaceObject = require('node:buffer') // CONCATENATED MODULE: external "node:path"
      const external_node_path_namespaceObject = require('node:path') // CONCATENATED MODULE: external "node:child_process"
      ...
      const external_node_url_namespaceObject = require('node:url') // CONCATENATED MODULE: ./node_modules/execa/node_modules/path-key/index.js
      ...
      function npmRunPath(options = {}) {
        ...
        const cwdString =
          cwd instanceof URL
            ? external_node_url_namespaceObject.fileURLToPath(cwd)
            : cwd
        ...
        result.push(
          external_node_path_namespaceObject.resolve(cwdString, execPath, '..')
        )

        return [...result, path_].join(
          external_node_path_namespaceObject.delimiter
        )
      }

      function npmRunPathEnv({
        env = external_node_process_namespaceObject.env,
        ...options
      } = {}) {
        env = { ...env }
      ...
      const SIGRTMIN = 34
      const SIGRTMAX = 64 // CONCATENATED MODULE: ./node_modules/human-signals/build/src/core.js
      ...
      const getSignalByNumber = function (number, signals) {
        ...
        const { name, description, supported, action, forced, standard } =
          signal
        return {
          [number]: {
            name,
      ...
      const findSignalByNumber = function (number, signals) {
        const signal = signals.find(
          ({ name }) => external_os_.constants.signals[name] === number
        )
      ...
      const signalsByNumber = getSignalsByNumber() // CONCATENATED MODULE: ./node_modules/execa/lib/error.js
      ...
      const isSigterm = signal =>
        signal === external_node_os_namespaceObject.constants.signals.SIGTERM ||
      ...
      const timeoutKill = (spawned, signal, reject) => {
        spawned.kill(signal)
        reject(
          Object.assign(new Error('Timed out'), { timedOut: true, signal })
        )
      }
      ...
      const command_joinCommand = (file, args) =>
        normalizeArgs(file, args).join(' ')

      const command_getEscapedCommand = (file, args) =>
      ...
      const handleArguments = (file, args, options = {}) => {
        ...
        if (
          external_node_process_namespaceObject.platform === 'win32' &&
          external_node_path_namespaceObject.basename(file, '.exe') === 'cmd'
        ) {
          // #116
          args.unshift('/q')
        }
      ...
      const handleOutput = (options, value, error) => {
        if (
          typeof value !== 'string' &&
          !external_node_buffer_namespaceObject.Buffer.isBuffer(value)
        ) {
      ...
      function execa(file, args, options) {
        ...
        let spawned
        try {
          spawned = external_node_child_process_namespaceObject.spawn(
            parsed.file,
            parsed.args,
            parsed.options
          )
          ...
          const dummySpawned =
            new external_node_child_process_namespaceObject.ChildProcess()
  ...
  ;(() => {
    ...
    const run = async () => {
      try {
        ...
        const matrix = groups.reduce(
          (acc, [pkg, { location, parallelGroups }]) => {
